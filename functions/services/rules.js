...
const rankHand = cards => {
  ...
  for (const card of cards) {
    const r = convertToNumber(card[0])
    result[r] = result[r] || []
    result[r].push(card)
  }
...
const value = (ranked, primary) => {
  ...
  for (const rank of ranked) {
    const r = convertToNumber(rank[0][0])
    const v = (r < 10 ? '0' : '') + r
...
const evalFront = (hand1, hand2) => {
  ...
  if (res1.handName === 'xam_chi' && res2.handName !== 'xam_chi') { return Number(hand1[0][0]) }
...
const evalMid = (hand1, hand2) => {
  ...
  if (res1.handName === 'cu_lu' && res2.handName !== 'cu_lu') { return 2 }
...
const isFoul = (back, mid, front) => {
  ...
  if (resFront.handType > resMid.handType || resFront.handType > resBack.handType) { return true }

  if (resFront.handType === resMid.handType && resFront.handRank > resMid.handRank) { return true }

  if (resFront.handType === resBack.handType && resFront.handRank > resBack.handRank) { return true }

  if (resMid.handType > resBack.handType) { return true }

  if (resMid.handType === resBack.handType && resMid.handRank > resBack.handRank) { return true }
